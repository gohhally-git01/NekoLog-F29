# =========================================================
#  "NEKOLOG-F29" GPSデータ解析プログラム  V7c By Hally
# =========================================================
...
def convert_f29_log(log_path, params):
    ...
    for i in range(1, len(cleaned)):
        ...
        if LINE_MODE == "split" and has_invalid_gap(p1["time"], p2["time"]):
            continue
            
        t1_str = p1["time"].strftime("%Y-%m-%dT%H:%M:%S")
        t2_str = p2["time"].strftime("%Y-%m-%dT%H:%M:%S")

        # 1. スルスル伸びる「線」のデータ（ピンを強制非表示にするため iconを空にする）
        feature_line = {
            "type": "Feature",
            "geometry": {
                "type": "LineString",
            ...
            "properties": {
                "times": [t1_str, t2_str],
                "style": {
                    "color": "#ff3366",   # 猫ちゃんの動線（ピンク）
                    "weight": 5,
                    "opacity": 0.9
                },
                "icon": "circle",        # 過去ピンが出ないようにダミー指定
                "iconOptions": {"style": {"opacity": 0, "fillOpacity": 0}} # 完全に透明化
            }
        }
        anim_features.append(feature_line)

        # 2. 最先端を走る「動く点」のデータ（これによって今ココ！の1点だけが動きます）
        feature_point = {
            "type": "Feature",
            "geometry": {
                "type": "Point",
                "coordinates": [p2["lon"], p2["lat"]]
            },
            "properties": {
                "time": t2_str,
                "style": {
                    "color": "#d62728",       # 最先端の円のフチ（赤）
                    "fillColor": "#ffffff",   # 中の色（白）
                    "fillOpacity": 1.0,
                    "radius": 6               # 丸の大きさ
                },
                "icon": "circle",
        ...
        anim_features.append(feature_point)

    if anim_features:
        geojson_data = {
            "type": "FeatureCollection",
            "features": anim_features
        }
        
        TimestampedGeoJson(
            geojson_data,
            period="PT1S",          # 1秒ステップ
            add_last_point=False,   # ←ここをFalseにすることで、お節介な自動青ピンを完全に消滅させます！
            transition_time=150,
            auto_play=False,
            loop=False,
            max_speed=10
...
class F29GUI:
    def __init__(self, root):
        ...
        # プロファイル
        self.preset_var = tk.StringVar(value="Cat_ST")
        tk.Radiobutton(config_frame, text="Nekolog Standard", variable=self.preset_var, value="Cat_ST", command=self.apply_preset).grid(row=0, column=0, sticky="w")
        tk.Radiobutton(config_frame, text="Walking Standard", variable=self.preset_var, value="Walking", command=self.apply_preset).grid(row=0, column=1, sticky="w")
        tk.Radiobutton(config_frame, text="Hiking log SP", variable=self.preset_var, value="Hiking", command=self.apply_preset).grid(row=0, column=2, sticky="w")
        tk.Radiobutton(config_frame, text="Drive log SP", variable=self.preset_var, value="Driving", command=self.apply_preset).grid(row=0, column=3, sticky="w")
    ...
    def apply_preset(self):
        # ★ 共有いただいたパラメータ表の数値を1点残さず完全に反映しました
        presets = {
            "Cat_ST": { 
                "static_dist": "2.0",
                "static_time": "10.0",
